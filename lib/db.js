...
const PUBLIC_COURSE_ORDER = [
  "we-master-estetica",
  "induccion-colageno-microagujas",
  "plasma-pen-fibroblast",
  "master-wax-corporal",
  "master-men-brazilian-wax",
  "iv-therapy-oligoelementos",
  "flebotomia-nacional-prp-prf",
];
...
export function getActiveCourses() {
  return readStore()
    ...
    .sort((a, b) => {
      const orderA = PUBLIC_COURSE_ORDER.indexOf(a.slug);
      const orderB = PUBLIC_COURSE_ORDER.indexOf(b.slug);
...
export function createEnrollment({ courseId, fullName, email, phone, message = "" }) {
  const store = readStore();
